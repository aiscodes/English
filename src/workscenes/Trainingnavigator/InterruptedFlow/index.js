...
const InterruptedFlowScreen = ({ user, vocabulary, learnVocabulary, vocabularyRoutine, learnRoutine, wordsUpdateRoutine, dataStatRoutine, wordStatRoutine }) => {
  const navigation = useNavigation();

  const doInterrupt = () => {
    CalcResults(user, vocabulary, learnVocabulary, vocabularyRoutine, learnRoutine, wordsUpdateRoutine, dataStatRoutine, wordStatRoutine);
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <TouchableOpacity style={{width:'100%'}} onPress={() => navigation.goBack()}>
          ...
          >
            <Text style={[styles.buttonText, { marginTop: -10 }]}>Вернуться в тренировку</Text>
          </ImageBackground>
        </TouchableOpacity>
        <TouchableOpacity
          onPress={doInterrupt}
...
const mapDispatchToProps = { vocabularyRoutine, learnRoutine, wordsUpdateRoutine, dataStatRoutine, wordStatRoutine };
export default connect(mapStateToProps, mapDispatchToProps)(InterruptedFlowScreen);
