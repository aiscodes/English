...
import { View, Text, Dimensions, StyleSheet, TouchableOpacity, ImageBackground, BackHandler } from "react-native";
import CalcResults from "../../Actions/results";
import { vocabularyRoutine, learnRoutine } from "../../../store/wordsReducer";
import { countStatRoutine, statStatRoutine, dataStatRoutine, wordStatRoutine } from '../../../store/statReducer';
...
const TrainingCompleteScreen = ({ user, vocabulary, learnVocabulary, vocabularyRoutine, countStatRoutine, statStatRoutine, wordsUpdateRoutine, newCount, stat, dataStatRoutine, wordStatRoutine}) => {
  ...
  const waitCalc = async () => {
    const { count, experience } = await CalcResults(user, vocabulary, learnVocabulary, vocabularyRoutine, learnRoutine, wordsUpdateRoutine, dataStatRoutine, wordStatRoutine);
    // setWordCount(count);
    // setExp(experience);
    ...
    stat[i].countTrue += count;
    stat[i].count += count;
    statStatRoutine({ stat });
    navigation.navigate("Statistics2", { wordExp: experience });
  };
...
const mapDispatchToProps = { vocabularyRoutine, learnRoutine, countStatRoutine, statStatRoutine, wordsUpdateRoutine, dataStatRoutine, wordStatRoutine };
export default connect(mapStateToProps, mapDispatchToProps)(TrainingCompleteScreen);
