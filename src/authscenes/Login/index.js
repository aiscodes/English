...
import { View, Text, TextInput, TouchableOpacity, Platform, StyleSheet, ScrollView, ImageBackground, Dimensions, ActivityIndicator, Alert } from "react-native";
...
const Login = ({ navigation, userRoutine }) => {
    ...
    const getTextEmail = (text) => {
        setEmail(text)
        const logg = text && validator.isEmail(text);
        setEmailOk(logg);
        setErrorButton(!(logg && isPasswordOk));
    }
    const getTextPwd = (text) => {
        setPassword(text)
        const logg = text && text.length >= minPassLen;
        setPasswordOk(logg);
        setErrorButton(!(logg && isEmailOk));
    }
        ...
        mode
            ...
            : <KeyboardAwareScrollView style={Platform.OS == "android" ? styleIOS.container : styleIOS.container}>
                <ScrollView style={Platform.OS == "android" ? styleIOS.container : styleIOS.container}>
                    ...
                    <View>
                        <Text style={ Platform.OS == "android" ? styleIOS.FastLoginSocialNetworks : styleIOS.FastLoginSocialNetworks }>
                            Быстрый вход через:
                        </Text>
                        <View style={ Platform.OS == "android" ? styleIOS.SocialNetworks : styleIOS.SocialNetworks }>
                    ...
                    <TextInput
                        onChangeText={getTextEmail}
                        placeholder="Электронная почта "
                        placeholderTextColor='#BDBDBD'
                        style={[styleIOS.Input, { borderBottomColor: `${isEmailOk ? 'green' : 'grey'}` }]}
                        keyboardType='email-address'
                        onSubmitEditing={onLoginSubmit}
                    />
                    <View style={{ flexDirection: "row", alignSelf: "center" }}>
                        <TextInput
                            onChangeText={getTextPwd}
                            placeholder="     Пароль "
                            placeholderTextColor='#BDBDBD'
                            style={styleIOS.InputPass}
                    ...
                    {errorButton == true
                        ...
                        >
                            ...
                            >
                                {isLogin
                                    ...
                                    : <Text style={ Platform.OS == "android" ? styleIOS.buttonTextA : styleIOS.buttonText }>
                                        Войти
                                        </Text>}
                            </ImageBackground>
                        </TouchableOpacity>
                        ...
                        >
                            ...
                            >
                                {isLogin
                                    ? <ActivityIndicator size="large" color="#fff" style={{ marginBottom: 10 }} />
                                    : <Text style={Platform.OS == "android" ? styleIOS.buttonText : styleIOS.buttonText}>
                                        Войти
                                      </Text>}
                            </ImageBackground>
                        </TouchableOpacity>
                    ...
                    <TouchableOpacity onPress={() => navigation.navigate("LoginPassword", { email })}>
                        <Text
                            style={
